...
- Branch: `main`
- Latest implemented provider-aware redirect foundation: `06c0d53`.
- Latest implemented Milestone 6 filter empty-state recovery: `eb6f1a2`.
- Latest provider-selection decision record: `50c2db8`.
...
- Latest 2026-07-14 local verification: `npm run build` passed; `npm run lint` passed with the existing `LegalPage.jsx` warning; local API smoke found LHR -> DXB Amadeus offers with `AZ` and `LH` carrier segments; local Browser QA rendered 12 result cards with `AZ / Lufthansa`, two airline logo badges on multi-carrier cards, `Book via Aviasales`, `Lufthansa (LH)` in the airline filter, and 8 of 12 offers after filtering by `LH`.
- Latest 2026-07-14 live verification: `https://tryfarely.com` served `assets/index-V_jUzpOJ.js`; `/api/health` reported `demoFallbackEnabled:false`; live `/api/flights` for LHR -> DXB returned Amadeus offers with `AZ` and `LH` carrier segments; live Browser QA rendered 12 result cards with `AZ / Lufthansa`, two airline logo badges on multi-carrier cards, `Book via Aviasales`, `Lufthansa (LH)` in the airline filter, and 8 of 12 offers after filtering by `LH`; `/api/deals/flight` returned a tracked `302` to Aviasales.
- Latest 2026-07-15 local verification: `npm run build` passed; `npm run lint` passed with the existing `LegalPage.jsx` warning; local `/api/flights` for LON -> DXB returned 25 Amadeus offers with `dealPartnerName:"Aviasales"`; local in-app Browser QA showed a stale LON -> DOH departure-airport filter reset from `Filters (1)` back to `Filters` after changing destination to Dubai, then rendered 12 LON -> DXB result cards with 12 top booking strips and no console warnings/errors.
- Latest 2026-07-15 live verification: `https://tryfarely.com` served `assets/index-3otPvE22.js`; `/api/health` reported `demoFallbackEnabled:false`; live `/api/flights` for LON -> DXB returned 25 Amadeus offers with `dealPartnerName:"Aviasales"`; live Browser QA showed a stale LON -> DOH departure-airport filter reset from `Filters (1)` back to `Filters` after changing destination to Dubai, then rendered 12 LON -> DXB result cards, 12 top booking strips, `Book via Aviasales`, and no page console warnings/errors.
- Latest 2026-07-16 local provider-redirect verification: `npm run build` passed; `npm run lint` passed with the existing `LegalPage.jsx` warning; `npm run audit:hourly` passed against `https://tryfarely.com`; local `/api/deals/flight` smoke checks confirmed explicit `provider=travelpayouts` and disabled `provider=kiwi` requests still return tracked `302` Aviasales redirects, while click metadata records the resolved provider and requested provider.
- Latest 2026-07-16 live provider-redirect verification: pushed to GitHub `main`; live `/api/health` reported `demoFallbackEnabled:false` with Travelpayouts configured; live `/api/deals/flight?provider=travelpayouts` returned a tracked `302` to Aviasales; post-push `npm run audit:hourly` passed against `https://tryfarely.com`.
- Latest 2026-07-17 filter empty-state recovery verification: implementation commit `eb6f1a2` was pushed to GitHub `main`; `https://tryfarely.com` served `assets/index-CLsCnw_Z.js`; `npm run build` passed; `npm run lint` passed with the existing `LegalPage.jsx` warning; local desktop browser audit rendered exact-date results, forced a no-match filter state, showed `No fares match these filters`, and recovered after `Show all offers`; a targeted live production-bundle UI check on `https://tryfarely.com` with a controlled `/api/flights` response rendered the new no-match reset path, restored result cards, had no console warnings/errors, and had no horizontal overflow. Full live exact-date provider verification was blocked because live `/api/flights` returned Amadeus test-environment `500` responses across checked routes.
...
- Result cards and airline filters now account for every carrier in visible multi-segment offers, so secondary carriers such as `LH` can be displayed and filtered when an offer is sold across multiple airlines.
- Result filters now reset when the underlying search context changes, so a stale airline, airport, price, or baggage filter does not silently hide valid fares after the traveller changes route, date, trip type, cabin, passengers, or selected Flexible date.
- Filtered result empty states now include a `Show all offers` action so travellers can recover immediately when budget, stops, airport, time, or duration filters remove every fare.
...
- `/api/deals/flight` is now provider-aware behind server config. The current default remains Aviasales/Travelpayouts, and disabled or unconfigured provider requests fall back to the primary path instead of showing an unapproved partner.
...
- On 2026-07-17, live exact-date `/api/flights` checks returned Amadeus test-environment `500` responses across the small route set tested, so UI-only live verification used a controlled response while provider health remains a separate blocker.
...
Next engineering priority: after Kiwi.com approval and its redirect/deep-link template are confirmed, add the approved Kiwi template to the existing server-controlled provider redirect config, verify tracking internally, and only then expose a visible comparison row.
