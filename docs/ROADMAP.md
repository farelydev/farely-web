...
- Filter empty-state recovery: implemented, pushed, deployed, and verified on the live production bundle with a controlled flight response on 2026-07-17 so no-match filtered results show a clear `Show all offers` reset action. Full live exact-date provider verification was blocked by Amadeus test-environment `500` responses.
...
- First second-provider candidate: Kiwi.com, pending founder/account confirmation and approved redirect/deep-link template. The hidden provider-aware redirect foundation now exists, but the approved Kiwi template must be configured and verified before any visible comparison row is shown.
